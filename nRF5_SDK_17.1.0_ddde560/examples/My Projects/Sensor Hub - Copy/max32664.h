...
static uint8_t _resetPin = 7;
static uint8_t _mfioPin = 8;
...
enum IDENTITY_INDEX_BYTES {
  ...
  static uint8_t bpmSenArrTwo[MAXFAST_ARRAY_SIZE + MAXFAST_EXTENDED_DATA + MAX30101_LED_ARRAY];
  ...
  uint8_t writeByte3(uint8_t, uint8_t, uint8_t, uint8_t);
  ...
  // indicating what you want to do, a delay, and then a read to confirm positive
  // transmission.
  uint8_t writeByte2(uint8_t, uint8_t, uint8_t, uint16_t);
  ...
  uint8_t readByte2(uint8_t, uint8_t, uint8_t);
  ...
  uint8_t readMultipleBytes2(uint8_t, uint8_t, uint8_t, const size_t, uint8_t userArray[]);

  // Needs comment - INCOMPLETE
  uint8_t readFillArray(uint8_t, uint8_t, uint8_t, uint8_t array[]);

  void max32664_setup(uint8_t mode,uint8_t configMode);
